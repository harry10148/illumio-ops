...
- **Rich report engine** — three traffic report profiles (`report traffic` for plain traffic facts, `report security` for the 15-module security & risk analysis, `report inventory` for network inventory) plus audit, policy-usage, VEN-status, and Enforcement Readiness (`report readiness` — which app/env can safely move to enforcement next) reports, rendered to **HTML / CSV / XLSX**.
