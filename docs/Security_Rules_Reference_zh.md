...
**偵測內容：**
偵測中危連接埠上 `policy_decision = 'potentially_blocked'` 的流量，代表分段規則存在但工作負載仍在**可見性/測試模式**，封鎖**尚未生效**。

監控連接埠包括：SSH (22)、NFS (2049)、FTP (20/21)、mDNS (5353)、LLMNR (5355)、HTTP (80)、WSD (3702)、SSDP (1900)、Telnet (23)。

**重要性：**
`potentially_blocked` 是最具欺騙性的狀態：規則在 PCE 中看起來已建立，但流量仍自由通過。這是「我們有政策但仍遭入侵」最常見的根本原因。
...
**重要性：**
`potentially_blocked` 是 Illumio 部署中最危險的狀態：**分段規則在 PCE 中存在，但工作負載處於可見性/測試模式，封鎖並未生效**，流量自由通過。這是統計上最常見的「我們有微分段政策但仍遭入侵」根因——政策已正確撰寫，但從未強制執行。
...
- 這是投資報酬率最高的修復措施：規則已存在，只需更改強制執行模式
...
| 面向 | 權重 | 說明 |
|:---|:---|:---|
| Policy Coverage | 40 分 | 具有 `allowed` 政策的流量百分比 |
| Ringfence Ratio | 20 分 | 來源與目的地具有相同應用標籤的流量百分比 |
| Enforcement Mode | 20 分 | 受管工作負載處於強制執行模式的百分比 |
| No Blocked Flows | 10 分 | 反向衡量 blocked/PB 流量的比例 |
