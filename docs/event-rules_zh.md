...
> 除了上述輪詢式的 `system` health 狀態檢查外，PCE 本身約每分鐘還會發出 `system_health` 事件（Severity 為 **Warning**、**Error** 或 **Fatal**）；監控這些事件是 Illumio 建議的最佳實務（best practice）。（來源：Illumio「Events Described — Recommended Events to Monitor」。）
