...
2. **policy_decision（`pd`）**：flow 的 `pd` 數值化為 `0=allowed`、`1=potentially_blocked`、`2=blocked`（無 `pd` 欄時由 `policy_decision` 字串推斷）。規則 `pd` 預設為 `-1`（event 類）或 `3`（traffic 類）；**`-1` 與 `3` 皆代表「任意 PD」**，其餘需精確相等才命中。
