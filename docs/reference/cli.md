...
> **`pdf` 選項已移除**（2026-07-17）：`--format` 合法值為 `html`／`csv`／`xlsx`／`all`。
> 舊版曾接受 `pdf` 但從未有 exporter 實作（選了會靜默不輸出、結束碼仍 `0`），
> 現已從選項清單移除——傳入 `pdf` 會直接被 CLI 以無效選項拒絕。需要 PDF 時
> 用 `--format html` 後以瀏覽器列印為 PDF。歷史脈絡見
> [troubleshooting.md](../guide/troubleshooting.md) §4.3。
...
| `--format` | `[html\|csv\|xlsx\|all]` | `html` | 輸出格式。 |
...
| `--start-date` | TEXT (`YYYY-MM-DD`) | 無 | 開始日期。 |
| `--end-date` | TEXT (`YYYY-MM-DD`) | 無 | 結束日期。 |
| `--format` | `[html\|csv\|all]` | `html` | 輸出格式（此子命令不接受 `xlsx`）。 |
| `--output-dir` | PATH | `reports/` | 報表輸出目錄。 |
| `-h, --help` | | | 顯示說明並退出。 |
...
- [故障排除](../guide/troubleshooting.md) — 報表輸出異常等症狀導向排錯
