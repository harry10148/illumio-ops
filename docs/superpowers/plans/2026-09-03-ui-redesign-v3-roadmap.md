...
| 3B | `2026-09-03-phase3b-gui.md`（**下一步**：3A 已合入，可撰寫） | spec §5 token（`css/v3/tokens.css`＋components）→ §1 五區路由與 shell → §2 首頁 → §3 調查中樞（收件匣／詳情／流量帶條件／規則面板／行動）→ 政策／報表／設定區搬遷 → 登入頁；`design/v3/coverage.yaml` 重編＋coverage gate 改讀；每區完成即部署測試機 | 3A | 待寫 |
