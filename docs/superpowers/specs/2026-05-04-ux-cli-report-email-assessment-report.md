...
本 spec 是 design spec（`2026-05-04-ux-cli-report-email-assessment-design.md`）的「執行成果」對應文件。三階段流程已完成前兩階段：

1. **Design spec（已完成）** — 定義評估的結構、方法學、輸出模板、痛點卡格式、Visual Identity 候選方向。
2. **Implementation plan** — 由 writing-plans skill 產出，列出執行評估需要的步驟（`2026-05-04-ux-cli-report-email-assessment-execution-plan.md`）。
3. **本文件（評估執行成果）** — 依 plan 跑工具、填入 finding、產出推薦結論（Phases A–H 執行結果）。

每個 §X.Y 對應 design spec 同編號章節的「填入結果」；方法學 / rubric 定義皆引用 design spec，不在此重複。Mockups（M1/M4/M5）依 OQ-4 決策於本次執行跳過（詳見 §7）。
...
衝突解決：GUI/CLI 衝突優先 P1；Report/Email 衝突優先 P5。PersonaWeight：P1=3 / P2=2 / P5=2 / 其他=1。
...
- 進行中工作（h5 blueprint split、h6 settings reorg、Integrations redesign Task 6）→ 本評估假設完成

### §1.5 評估維度

- **UX**（行為 / 流程） → ui-ux-pro-max 10 priorities（§2.3）
- **Visual Identity**（外觀 / 美學） → frontend-design 5 維度 + Distinctiveness（§2.4）
...
| 外部依賴 | `grep -rn 'https?://' src/templates src/static src/alerts` | a7 違規清單與 vendor 化 |
| 重複度 | 目視 + `pylint --disable=all --enable=duplicate-code` | 跨檔重複 pattern |
| TTY 旗標命中 | `grep -rE 'isatty\(\)\|NO_COLOR\|sys\.stderr'` | CLI rubric 取證 |
...
- **PersonaWeight**：取此痛點觸及的最高 weight persona（P1=3 / P2=2 / P5=2 / 其他=1）
...
4. **Style Selection** — 一致風格、icon-set 一致、stroke 一致、filled vs outline 分層
...
- **Typography** — 字體選擇 + 配對（heading vs body vs mono），避免 generic
...
- **Backgrounds & Details** — gradient mesh / noise / 幾何 / 陰影 / grain
- **Distinctiveness** — 「unforgettable thing」存在嗎？目標 = 2（清楚工具感識別，不追求 art-piece 的 3）
...
每張痛點卡明示觸發了哪幾個 Gate（詳見 §4）。

### §2.7 Evidence collection 規範

每個 finding 必附：

- **代碼證據** — `file:line` 或 file glob（§3 各小節均已引用）
- **輸出證據（CLI）** — 量化指標表（§3.2.1 inventory + §3.2.2 consistency matrix）
- **視覺證據（GUI / Report / Email）** — 文字描述 + 量化 rubric 表（§3.1.2、§3.1.3、§3.3.3、§3.4.2）
- **Mockup 規範** — M1 light + dark、M4 light only、M5 light + dark（本次執行依 OQ-4 跳過，詳見 §7）
...
完整 4 候選評分表（Typography / Color / Motion / Spatial / Backgrounds / Distinct / 適用 P1 / P2 + 合計）見 §6.1 Step 1。詳細 spec sheet（B/C/D 各候選字型 / 色票 / motion / density / touch radius / risk）見 §6.1 Step 2。**採用方向：B（industrial-editorial）**，詳見 §6.1 Step 4 Adopted Direction Spec Sheet。
...
**Framework axis 5 Gate 評估（§3.1.5 摘要）**：Stay Vanilla + Design System 唯一不觸發 Gate 1/4（C1 offline 安全、P1 功能不破），其餘三者均需 Gate 1 驗證或 Gate 4 P1 衝擊評估。**推薦：Stay Vanilla + Design System（Track A 路線）**；Vue 3 在 Phase 4 前不推薦。
...
**Backend axis 5 Gate 評估（§3.1.5 摘要）**：Flask 現狀無 Gate 觸發（offline ✓，P1 不破）；FastAPI / Starlette 的 Gate 1 已確認（uvicorn + starlette wheels offline 可用）；Gate 4 P1 衝擊：Flask blueprint 平移至 ASGI 需驗證 Talisman CSP middleware 等效。**推薦：Flask（Phase 0-3 默認）→ FastAPI/Starlette（Phase 4 Track E conditional）**。
...
**掃描來源**：`src/cli/menus/`（main_menu, siem_cli, pce_cache_cli, rule_scheduler_cli）+ §3.2.1 inventory（24 commands）。

##### 選單結構

| 選單入口 | 層數 | 選項數（第一層） | back / ESC 機制 | breadcrumb | 狀態 |
|---|---|---|---|---|---|
| `main_menu.py` | 2（main → sub-menu） | 8（含 0=Exit） | `0` 回上層；無 ESC handler | 無 | 純 `print()` + `input()` |
| `siem_cli.py` | 2 | 9（含 0=Exit）+ 8a/8b submenu | `0` 回主選單；DLQ 子選單無明確 back | 無 | ANSI Colors + `print()` |
| `pce_cache_cli.py` | 2 | 6 + view-status submenu | `0` 回主選單；view-status 無 back | 無 | 純 `print()` + `input()` |
| `rule_scheduler_cli.py` | 1 | a/e/d/r 四個字母選項 | 無明確 back；loop until 輸入空白 | 無 | ANSI Colors + `print()` |

##### 關鍵不一致

1. **back 語義分裂**：`main_menu` / `siem_cli` / `pce_cache_cli` 用 `0` 返回；`rule_scheduler_cli` 用空白 Enter；無任何選單支援 ESC 鍵返回（b1 痛點核心）。
2. **Ctrl-C 行為**：全部 4 個 menu 入口均無 `try/except KeyboardInterrupt`，Ctrl-C 直接引發 Python 例外並終止程序（exit 0），無回父選單的可能。
3. **breadcrumb 缺失**：4 個選單均無任何層級指示。進入 `siem_cli → DLQ submenu` 後使用者無視覺線索確認自己的位置（b1）。
4. **狀態保留缺失**：選單間切換不保留任何 context（如上次選取的 SIEM destination 名稱），每次進入 submenu 均需重新輸入（b1）。
5. **輸入驗證不統一**：`safe_input()`（`_render.py`）在部分選單使用、部分選單直接裸 `input()`；錯誤輸入的回饋訊息格式不統一（b2）。
6. **14 個重複命令**：§3.2.1 確認 14/24（58%）命令在 menu 與 root CLI 兩處皆有入口，但 menu 版本用 `print()` 渲染，CLI 版本用 `rich`，行為不一致（b5）。

##### 與 CLI rubric 映射

| CLI rubric 條目 | menu 得分 | 主因 |
|---|---|---|
| §9 Navigation Patterns | 2 | menu 層級存在但 breadcrumb 缺失、back 語義分裂 |
| §1 Accessibility | 1 | Ctrl-C 直接終止，鍵盤逃脫路徑不明 |
| §7 互動 vs 非互動雙模 | 1 | menu 路徑無 --non-interactive fallback |
| §11 雙入口整合 | 1 | 14/24 命令重複實作，渲染不統一 |
...
**推薦**：L2（Track B）→ L3（Track C），不推薦 L4。

- L2 先行（共享輸出層）：Touch radius 中，可獨立 sprint ship，解 b3/b4/b6/b7 四痛點，不破壞現有命令結構。
- L3 後續（統一入口）：依賴 L2 共享層已到位；menu 轉為 `illumio-ops shell` mode，三支獨立 CLI 退為 deprecation alias。
- L4 排除：重寫成本 > 收益，P1 用戶體驗可透過 Track B+C 達標，無需全面換框架。
...
完整候選 spec sheet（Typography / Color / Spatial / Backgrounds / Distinct / 適用 P5 評分 + Email 子集說明）已於 §6.2 Step 2 展開。以下為摘要：

- **A（維持現狀）**：HTML 12/18、PDF 6/15；PDF Backgrounds 0/3 為最大缺口；Distinct 1；不推薦。
- **B（editorial-magazine）**：Source Serif 4 heading × Inter body；cover page + section divider + @page footer；Email table 佈局 + bulletproof CTA；總評 13/15；**採用**（詳見 §6.2 Step 4 Adopted Spec Sheet）。
- **C（data-journalism）**：DM Sans 字族 + 圖表主導版面；Email 圖表降級為 verdict table；總評 13/15；P5 適用「中」，圖表密度高需額外解讀；不推薦（現況 Report 並非純圖表驅動）。
- **D（corporate-formal）**：保守 serif + 低飽和配色；Distinct 1 與現狀無改善；合規場景適用，但不適合本 context；不推薦。

#### §3.3.5 痛點對應 finding

**c1（Report 摘要 / 長度）：**
- 4 份報告（audit / policy_usage / ven_status / traffic）均有 `mod00` exec summary 結構，但 standalone 可讀密度未驗證（樣本 HTML 輸出不可用，Task A.6 已跳過實際量測）。
- Traffic 報告 16 sections 無 TOC sidebar，閱讀節奏不明確。
- 跨報告連結：0（audit ↔ policy_usage ↔ ven_status 間無任何 `<a href>` 連結）。
- 對應 §4.13 卡（c1，P2）推薦：優化路線先行（exec summary 200 字 × 3 reports + cross-report sidebar），1 week 內可完成。

**c3（圖表閱讀性）：**
- VEN 餅圖 label hardcoded 英文（`ven_html_exporter.py:107`），繞過 i18n 管線。
- HTML chart（Plotly）無 `aria-label`，4 個 chart div 螢幕閱讀器不可用（§3.1.2 §10 Charts = 1）。
- PDF chart 缺 colorblind-safe palette（PDF Color = 1/3）；`chart_renderer.py` 調色板與 `report_css.py` token 未共享。
- i18n 不一致 4 項（nav vs section title 分裂、Managed 圖表 vs nav 不同、VEN chart hardcoded label、ALLOWED 全大寫 i18n bypass）。
- 對應 §4.14 卡（c3，P2）推薦：並行優化路線（4 點 patch，1 week），與 c1 同 sprint。
...
**評分來源**：`src/alerts/mail_wrapper.html.tmpl`、`src/alerts/line_digest.txt.tmpl`、`src/reporter.py`（Email builder）。

| 維度 | Score | 關鍵證據 |
|---|:---:|---|
| Typography | 1 | `font-family:'Montserrat',Arial,sans-serif`（mail_wrapper.html.tmpl:2）；Montserrat 在 email client 普遍不可用，退回 Arial；無 heading/body 字族對比；純 sans-serif 無 editorial 個性。 |
| Color | 1 | logo badge `#FF5500`（Illumio Orange）存在；summary card 背景 `#F3F0E9`（warm off-white）；無 signal 語義色票（CRITICAL/HIGH/MEDIUM/LOW 均無對應 badge 色彩）；severity 無色彩區分。 |
| Spatial | 2 | max-width wrapper 存在（wrapper 寬度有限制）；summary card 以 flex row 排列（Outlook 塌陷風險）；section spacing 以 padding 控制，有節奏感；但 layout 以 `<div>` + flex 為骨，Outlook 渲染不安全。 |
| Backgrounds & Details | 1 | `#F3F0E9` warm off-white 背景為唯一背景色處理；無 verdict band、無 section divider、無 cover 視覺元素；logo badge `background:#FF5500;border-radius:50%` 為唯一品牌識別細節。 |
| Distinctiveness | 1 | 品牌 token 僅有 `#FF5500` logo badge，其餘全走通用 email 灰階；無 signal color 語義、無 preheader、無 bulletproof CTA——整體為系統預設 notification email 外觀，無工具個性。 |
| **合計** | **6/15** | |

**主要 gap**：
- **Color（1/3）**：無 severity signal 色票（success/warning/danger/info 均缺席），收件人無法從視覺快速區分告警嚴重程度。
- **Typography（1/3）**：Montserrat 在 email 環境不可用；無 serif heading 對比，排版無層次。
- **Backgrounds & Details（1/3）**：缺 verdict band、section divider、品牌 header 色塊，視覺識別度低。
- **整體定位**：與 §6.2 B（editorial-magazine）方向落差最大，Draft D.2 決策（`<table>` layout + `Georgia,...,serif` + signal token inline bgcolor）即為補強此缺口的主要路徑。
...
**d2（Email 跨 client 顯示）：**
- `§3.4.2` 8 項 known-issue checklist：2/8 通過（inline CSS ✓、無圖片 ✓），6 項失敗。
- 最高風險：Outlook 365 flex 塌陷（`display:flex` 在 Word HTML engine 完全不支援）；`multipart/alternative` 缺失（`line_digest.txt.tmpl` 存在但從未作為 `text/plain` part attach）；Apple Mail dark mode 無 `color-scheme` meta 保護。
- 對應 §4.15 卡（d2，P1）：優化路線 5 點（table layout + bgcolor 雙軌 + color-scheme meta + multipart + font-stack 改 Georgia）1 week 可完成，2/8 → 7/8。

**d3（Email 主旨 / 摘要 actionability）：**
- `§3.4.4` 4 項 actionability checklist：0/4 通過。
- Subject 固定格式（`"Illumio PCE Ops Alert ({count} issue(s))"`），無 severity 前綴，inbox 排序失效；preheader 完全缺席，preview 行顯示無資訊的 "Official Alert Notification"；CTA 僅存在於 event 區段（條件性），health/traffic/metric 無 CTA；Why 層（severity badge + threshold）缺席。
- 對應 §4.16 卡（d3，P1）：優化路線 5 點（subject pattern + preheader + CTA 全覆 + Why badge + runbook link）1 week，0/4 → 4/4。
...
> **SKIPPED**（OQ-4 決策）：本次執行依 §8 OQ-4 結論（`Phase G 跳過 in this execution`）跳過 Visual Companion mockup 產出。M1 HTML 片段留待下游 implementation plan 產出；§6.1 Step 4 Adopted Direction Spec Sheet（industrial-editorial B 方向 CSS token）已提供充分的 before/after 設計規格，可作為 implementation 的直接依據。
...
> **SKIPPED**（OQ-4 決策）：同 M1。§6.2 Step 4 Adopted Direction Spec Sheet（editorial-magazine B 方向 CSS + @page 規則）已提供充分規格，包含 cover page、chapter rhythm、verdict band、tabular figures 全套設計規格，可作為 implementation 直接參考。
...
> **SKIPPED**（OQ-4 決策）：同 M1。§6.2 Step 4 Email subset 規格（`<table role="presentation">` + inline bgcolor + bulletproof CTA HTML 結構）已提供 implementation 所需完整 HTML pattern，含 Outlook 2016/2019 兼容性說明。
...
- XLSX 報告 GUI 不可見（`gui/__init__.py` 未傳 `traffic_report_profile` 是已知成因）→ bug fix sprint
- a6 HTTPS layout 破版修復 → 可靠性 sprint（hand-off 詳見 §3.1.0 a6；本評估已確認根因 CSP `font-src 'self'` 阻擋 Google Fonts CDN）

### i18n 資料補齊

不是結構 / UX 議題；OQ-2 已開放重組空間，列為 Track A/D 配合空間：

- `gui_sched_*` / `daily_report` / `daily-audit-report` / `gui_time_minutes_ago` 等 missing key → i18n sprint
- `traffic_report_profile` 參數遺漏 → Integrations redesign Task 6 收尾
- VEN chart label i18n bypass（`ven_html_exporter.py:107`）→ 列為 c3 優化路線 item 1（§4.14）

### 進行中工作（本 spec 假設完成）

評估執行時以下工作已完成（依 §1.4 不在範圍聲明）：

- `h5-gui-blueprint-split`：9 個 blueprint factory 分拆完成（commit `1fa8c61`）
- `h6-settings-rename-reorg`：CLI settings 拆至 `src/cli/menus/`，10 步完成（commit `31d4715`）
- Integrations UI/UX redesign Task 6（OQ-3 預設視為延伸 pre-condition，假設完成）

### 技術選型升級（值得獨立 spec）

- Vue / React 完整 SPA framework swap：§3.1.4 候選列出但不推薦（P1 不需 SPA，offline cost > 收益）
- Click + Rich + Typer 完全重寫 CLI（OQ-6 已解：不推薦，Track B+C 已足夠達 P1 目標）
- FastAPI / Starlette backend swap：§3.1.5 Track E（conditional），Phase 4 條件啟動

### 效能 profiling（不在 UX 評估範圍）

- §4.1（a1）給 P0 TTI 估算「> 3s」基於 bundle 大小推論（291 KB JS + 127 KB HTML），非 RUM 實測
- 實測 Lighthouse / WebPageTest → 獨立 implementation 任務
- CLI 命令 startup latency（Python import overhead）→ 獨立效能 sprint

---

## §10 Glossary

- **Pre-condition** — 必須先解決才能進行其他評估的痛點（直接違反硬約束 / 阻斷其他 finding 採集）。本評估中為 a6（HTTPS layout 破版）+ a7（external resources 違反 C1）。
- **Touch radius** — 一個改動會影響的程式碼範圍。小（< 5 檔）/ 中（單一模組）/ 大（跨模組）。
- **5 Gate** — §2.6 用以判定「優化 vs 重構」推薦的五個關卡：Offline 友善度 / 多痛點共因 / Touch radius / Persona 衝擊 / Reversibility。每張痛點卡（§4）均列出 Gate 評估結果。
- **Track** — §5.2 的 bundled refactor unit，一條 Track 對應一個結構性改動，可解多個痛點：Track A（Visual System）/ Track B（CLI Output Layer）/ Track C（CLI Entry Unification）/ Track D（Email System）/ Track E（Backend Async，conditional）。
- **Persona Weight** — §2.2 評分公式中的 persona 權重。P1 網管=3 / P2 SOC=2 / P5 主管=2 / 其他=1。
- **Distinctiveness** — frontend-design 中用以判定一個介面是否有「unforgettable thing」的整體分。本評估對 ops 工具的目標 = 2，不追求 art-piece 的 3；過度 distinctive 對 ops 工具會犧牲操作效率。
- **TTY-specific 12 條** — §2.5 自訂的 CLI rubric，補 ui-ux-pro-max 不涵蓋的 CLI 議題（命令文法一致性 / 能力偵測★ / Composability★ / Exit codes★ / Idempotency / 配置層級 / 互動 vs 非互動 / 長任務 / help / completion / 雙入口 / Error actionability）。
- **R-yes** — 「每個痛點都列出『優化路線 + 重構路線』雙方案」的設計原則。本評估 §4 的 16 張痛點卡均遵循此原則。
- **BLOCKED** — §3.1.0 a7 P0 狀態：login.html 第 7–8 行直接從 Google Fonts CDN 載入 Montserrat 字型，在離線環境（C1 constraint）下阻斷字型載入，須 vendor 化後解除。
- **Bulletproof CTA** — Email 中以 `<a>` + inline border + bgcolor attribute 雙軌實現的按鈕，在 Outlook 2007-2019 Word HTML engine 下不依賴 CSS `background-color`（因 Outlook 忽略此屬性），改用 `bgcolor` HTML attribute 作為 fallback。
...
- radon — Python CC / MI 複雜度分析工具
- `no-color.org` — `NO_COLOR` 環境變數標準（CLI 色彩停用慣例）

### 相關專案 spec / plan

- `docs/superpowers/specs/2026-05-04-ux-cli-report-email-assessment-design.md`（本報告對應的 design spec）
- `docs/superpowers/plans/2026-04-25-integrations-ui-ux-redesign.md`（OQ-3 引用，Task 6 假設完成）
