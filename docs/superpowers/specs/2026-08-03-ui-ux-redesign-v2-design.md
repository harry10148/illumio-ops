...
| 視覺 | 3 方向比稿後定案 → **2026-08-03 使用者選定方向 B｜Ops 深色監控台**（深色優先、高資料密度、等寬數字；亮色模式保留深色頂欄 chrome，實作期可再調） |
...
- 全域健康列（**2026-08-05 使用者裁決修訂：僅總覽區顯示**，不再全頁常駐）：5 顆語意燈——Daemon/排程、PCE 連線、Cache lag、SIEM 管線、告警管道。點開 popover 摘要 + 前往詳情連結。資料源 `/api/status` + `/api/dashboard/overview`，零新後端
