...
判準：config 帶 `pce_profiles` 且 `active_pce_id` 指向的 profile 的 `url`/`org_id`
與 `api` 區塊**不一致** → 該機器的 cache 幾乎確定混了兩台 PCE 的資料。

即使一致也不能完全排除（可能切走又切回），但不一致是可判定的強訊號。
...
| 情形 | 判定 | 輸出 |
|---|---|---|
| 無 `pce_profiles` | 鍵不存在 | 靜默（不印任何東西） |
| 有 profile，且 active 與 `api` 一致 | url 與 org_id 皆相同 | 一行提示：profile 已移除、憑證仍在檔案裡 |
| 有 profile，且 active 與 `api` **不一致** | 任一不同 | **顯著警告**（見 4.2） |
| 有 profile 但 `active_pce_id` 找不到對應 | id 不在清單內 | 比照不一致處理 |
...
1. **發生了什麼**：這台機器的 cache 可能同時含有兩台 PCE 的資料，沒有標記可以區分
2. **影響**：建立在 cache 上的報表、告警與查詢，其歷史基準橫跨兩個環境
3. **兩個 PCE 分別是誰**：active profile 的 url/org 與 `api` 區塊的 url/org，兩者並列
4. **可照抄的處理指令**，且順序正確：

```
sudo systemctl stop illumio-ops
sudo -u illumio-ops <INSTALL_ROOT>/python/bin/python3 -c "from src.cli import cli; cli()" \
    config login --url <要用的 PCE> --org-id <org> --pce-target-change flush
...
若 4.1 判定為不一致，額外提示：封存日檔同樣可能跨兩台 PCE，沖 cache 不會動到它們，
...
`install.ps1` 需要 4.1–4.4 的等價實作，以及 2.2 指出的既有缺口（`http_redirect_port`
...
- 升級檢查的單元測試（以暫存 config 檔驅動）：四種情形各一，斷言輸出**包含**必要元素
  而非逐字比對（文案會變）
- **URL 正規化**：`https://h:8443` vs `https://h:8443/` vs `https://h:8443/api` 判為同一台；
  不同 host 或不同 org 判為不同
