...
## 附錄 B：`dst_bi`/`dst_bo` 的語意已由 PCE syslog 證實（2026-08-24）

附錄 A.2 記載的未解問題（Codex P1）**已解決**。決定性證據來自 PCE 直送 Graylog
（`172.16.15.105:9000`，input `illumio`，SyslogUDPInput:5514）的 CEF flow_summary。
該格式**同時**帶累計值與增量值，欄位皆有官方定義：

```
CEF:0|Illumio|PCE|25.2.40|flow_potentially_blocked|...|
  in=72533 out=92590            <- 累計（CEF 標準欄，對應 dst_tbi/dst_tbo）
  cn2=82  cn2Label=dbi          <- 該取樣區間增量
...
取 12 小時內回報 >= 3 次的長命 flow，比對「累計值的相鄰差」與「該次回報的 `dbi+dbo`」：

| flow | 回報次數 | 結果 |
|---|---|---|
| `172.16.15.254 -> 172.16.15.135:2055` | 8 | 每次 `累計差 == dbi+dbo`，零例外 |
| `172.16.15.122 -> 192.168.10.30:9290` | 6 | 同上 |
| `172.16.15.51 -> 172.16.15.161:0` | 8 | 同上 |
| `172.16.15.51 -> 172.16.15.102:0` | 8 | 同上 |
...
1. **`in`/`out`（= `dst_tbi`/`dst_tbo`）是持續累加的計數器**，不是單一取樣區間內的量。
   官方對 `dst_tbi` 的字面定義「total ... **in the latest sampled interval**」描述的是
   「該筆紀錄發出時的累計值」，不是「累計範圍限於該區間」——實測直接否證後者。
2. **Codex P1 所擔心的「`dst_bo` 可能是區間值」不成立。** §2.3 的 span 分桶推論方向正確。
3. `dbi`/`dbo` 是該區間的精確增量；官方頻寬公式 `dbo/ddms` 對 syslog 來源可直接計算。
4. 附錄 A.2「在 P1 解決之前本 spec 不進入實作」的封鎖條件**解除**。

### B.3 殘餘的一點不確定

已證實的是「累加、且跨區間連續」。**未直接證實**的是「累加的起點恰為 `first_detected`」——
syslog 紀錄不帶 `first_detected`。但兩件事支持它：計數器連續累加至少橫跨觀測到的所有區間；
且 async 端的 span 分桶（§2.3）顯示位元組量受 `timestamp_range` 上限約束。以
`last_detected - first_detected` 為分母因此是**目前可得的最佳估計**，其誤差方向為
「若真實起點晚於 `first_detected`，則分母偏大、速率偏低估」——偏保守，不會高估。

### B.4 附帶發現：cache 已六天看不到 172.16.15.x 的流量（獨立問題）

syslog 證明 PCE 此刻仍在接收並轉送 `172.16.15.254 -> 172.16.15.135:2055` 等 flow（累計 595 MB），
但本機 cache 對這三條 flow 的最後一列都停在 **2026-08-18 02:35**，之後 ingest 進來的
全是 `10.1.x`/`10.3.x`/`172.16.3.x`。

| flow | cache 列數 | cache 最後 last_detected | 當時最大 bytes |
|---|---|---|---|
| `172.16.15.254 -> 172.16.15.135` | 3 | 2026-08-18 02:35:00 | 116,179,556 |
| `172.16.15.122 -> 192.168.10.30` | 7 | 2026-08-18 02:24:59 | 9,723,471 |
| `172.16.15.51 -> 172.16.15.161` | 3 | 2026-08-18 02:35:10 | 2,890,776 |

**這不是本 spec 造成的，也不是 2F-1 造成的**（2F-1 於 08-23 才合入），但影響面比頻寬公式更大：
建立在 traffic cache 上的每一份報表、每一條告警、每一次查詢，都已對這批 workload 盲眼六天。
且這正是「bytes 全為 0」的真正成因——不是 EDC 沒開（EDC 實測正常：該組流量存在時 47–65% 帶位元組），
而是**帶位元組的那批 workload 整組從 cache 的視野裡消失了**。需另案查證。

---

## 附錄 C：「沒有位元組 counter」是正常狀態，且可由 `state` 判定（2026-08-24）

使用者指出：本來就不是每條連線都會有頻寬 counter。實測證實，並找到判定依據。

### C.1 欄位永遠存在，數值分不出「沒量」與「量到 0」

對 20,000 列 `raw_json` 實測：

| 情況 | 列數 |
|---|---|
| `dst_bi`/`dst_bo` 欄位不存在 | **0** |
| 欄位存在但為 0 | 18,906 |
| 欄位存在且 > 0 | 1,094 |

async query **從不省略**這兩個欄位，一律送 0。因此**單看數值無法區分「未量測」與「量測結果為零」**。
（syslog 路徑相反：無 counter 時 `in=`/`out=` 整個欄位不出現，例如 ICMP flow。）

### C.2 `state` 相關但**不是**可靠判定依據（2026-08-24 更正）

初稿把 `state` 當成「有沒有量到」的判定依據。**再取一個環境實測後，該結論不成立**——
兩個環境的 `snapshot` 差了八倍：
...
**設計不得依賴 `state` 判斷 counter 有無。** 目前沒有任何欄位能在 async query 的資料上
乾淨區分「未量測」與「量測到零」（欄位永遠存在且為 0，見 C.1）；syslog 路徑可以
（無 counter 時 `in=`/`out=` 整個欄位不出現），但那不是本專案的取值來源。

**實作結論：`bytes == 0` 一律視為「速率不可得」，不試圖分辨成因。** 對 volume 保守
（少報一筆零流量，不會誤報數字），對 bandwidth 是必要的。

### C.3 對設計的三個影響

1. **「沒有頻寬」是第一級的正常狀態，不是錯誤。** 顯示「—」而非 `0.00 Mbps` 因此不是
   美觀選擇，而是唯一誠實的做法——數值本身分不出兩者。
2. **`calculate_volume_mb` 有同一個缺陷，本 spec 原本漏了。** 它對缺 counter 的列回 0.0，
   下游印成 `0.00 MB (Total)`（真機封存查詢實見此輸出），把「沒量」講成「量到零」。
   volume 沒有分母、不是推測值——但「零」與「不可得」的混淆與頻寬同類，須一併處理。
3. **驗收的期望值只能取寬區間**：在有真實流量的 PCE 上，兩端皆 managed 的 flow 約有
   **六到七成**帶 counter（兩個環境實測 58–67%，`snapshot` 除外差異更大）。這個數字
   隨環境與時段變動很大，只能當「明顯偏離才需要查」的粗略參考，不可寫成硬性斷言。

### C.4 本機環境的限制（影響驗證，不影響設計）

本 appliance 目前指向 `tpce.lab.local`（172.16.15.131），其 139 個 managed workload 全為
`10.1.x`/`10.3.x` 模擬環境，即時查詢 2 小時共 1,682 筆 flow、**bytes>0 為 0 筆**。
帶真實位元組的 `172.16.15.x` workload 位於另一台 `pce.lab.local`（172.16.15.130）——
即 Graylog 收到 syslog 的那台。§C.2 的表格因此取自 2026-08-18 之前（appliance 尚指向 .130 時）
留在 cache 裡的資料。

**本機 cache 同時含有兩台 PCE 的資料且無任何標記**（.130 至 2026-08-18，.131 之後），
封存檔亦然。這正是 2026-08-21「移除 PCE profile」案所加的 re-point 沖洗防護要防的情況，
但該防護晚於這次切換，攔不到。清理與否待使用者裁決。
