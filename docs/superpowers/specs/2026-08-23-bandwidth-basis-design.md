...
零值集中在 `state=snapshot`（18,906 筆零值中佔 16,910）。只看**兩端皆為 managed workload**
的流量（使用者指出這是最快看到真實流量的切法），依 `state` 拆開：

| state | 列數 | bytes > 0 | 比例 |
|---|---|---|---|
| `active` | 139 | 108 | **77.7%** |
| `timed out` | 131 | 108 | **82.4%** |
| `snapshot` | 2,321 | 285 | 12.3% |
| `closed` | 2,925 | 27 | 0.9% |

（對照：單邊 managed 3.3%、兩邊皆非 managed 30%/30 列。）
...
3. **驗收有了具體期望值**：在有真實流量的 PCE 上，`state` 為 `active`／`timed out` 的
   managed↔managed flow 應有約 **八成**算得出速率。明顯低於此值代表資料面有問題，
   而不是公式有問題。
