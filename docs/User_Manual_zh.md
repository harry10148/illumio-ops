...
| `report` | 從 CLI 產生報表 | `illumio-ops report traffic --format html` |
| `rule` | 檢視已設定的監控規則 | `illumio-ops rule list --type traffic` |
| `siem` | 管理 SIEM 目的地：測試、狀態、DLQ、重送、清除 | `illumio-ops siem test splunk-hec` |
...
> **SIEM 操作命令：** `illumio-ops siem test`、`illumio-ops siem status`、`illumio-ops siem dlq`、`illumio-ops siem replay`、`illumio-ops siem purge`。
...
# 安裝到 /opt/illumio-ops、註冊 systemd unit
sudo ./install.sh
...
- `config/alerts.json` — 你的告警規則引擎狀態（`{"rules": [...]}`）
...
# 3. 執行 install.sh — config.json、alerts.json（rules）與 rule_schedules.json 會保留
sudo ./install.sh
...
# 安裝到 C:\illumio-ops、註冊 IllumioOps Windows 服務
.\install.ps1
...
`config\alerts.json` 與 `config\rule_schedules.json`。
...
# 3. 執行 install.ps1 — config.json、alerts.json（rules）與 rule_schedules.json 會被保留
...
**綁定主機：** GUI 綁定位址透過命令列 `--host` 設定（預設 `0.0.0.0`）；不儲存於 `config.json`。
...
| Web GUI 無法啟動 | 相依套件未安裝 | **正式（離線 bundle）**：執行 `/opt/illumio-ops/python/bin/python3 /opt/illumio-ops/scripts/verify_deps.py` 後重新執行 `sudo ./install.sh`。**開發**：`pip install -r requirements.txt`（Ubuntu 22.04+ / Debian 12+ 須改用 venv） |
...
| 升級後：載入舊設定 | `config.json`、`alerts.json` 與 `rule_schedules.json` 都會作為操作者擁有檔案保留 | 與 `config.json.example` 比較並手動新增任何新欄位；保留 `alerts.json` 以保留自訂告警規則 |
