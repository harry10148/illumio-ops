...
| Security & Risk | `report security` | 綜合流量安全分析（policy decisions、ransomware 曝險、lateral movement、enforcement readiness 等 13 個模組）；規則發現與修復行動已併入單一「發現與行動」（Findings & Actions）章；橫向移動（Lateral Movement）章恰呈現 4 張摘要表（service 檢視、fan-out 來源、allowed lateral flows、attack paths）——主機層明細（IP talkers/配對、bridge nodes、可達節點、app 鏈）已自 HTML 精簡（明細保留於分析資料層，並已於 XLSX 匯出的 Lateral Movement 分頁提供）；固定 security_risk 取向；可帶 `--vuln-csv` 納入漏洞掃描（Qualys/Tenable） |
| Network & Traffic Inventory | `report inventory` | 資產／標籤治理導向報表，聚焦 7 章（Executive Summary、Label Hygiene、Policy Decisions、Cross-Label Matrix、Unmanaged Hosts、Application Ringfence、Change Impact）——不含流量總覽／分佈／頻寬章節（這些請見 `report traffic`）；Cross-Label Matrix 章 HTML 僅呈現 ENV/APP 兩維，ROLE/LOC 明細已自 HTML 精簡（保留於分析資料層，並已於 XLSX 的 Cross-Label 分頁提供）；Unmanaged Hosts 章恰 3 張表（主要 unmanaged 來源、接收 unmanaged 流量的 managed apps、合併版 exposed ports 含主要 unmanaged 來源）；Change Impact 章比對本次執行與同 profile 上次快照的態勢 KPI——首次執行顯示「無先前快照」提示，之後執行則渲染差異表；固定 network_inventory 取向 |
| Audit | `report audit` | 稽核事件與 policy 變更（支援 `--start-date`/`--end-date`）；Health/Users/Policy Changes 章的 HTML 各僅顯示最近 10 筆事件（完整清單——上限為分析資料層的 50 筆——見 XLSX 匯出） |
| VEN Status | `report ven-status` | VEN 狀態（online／offline）盤點，外加 unmanaged workloads（在 Illumio 中，「Unmanaged」是 Workload 狀態——以 IP 標記、未安裝 VEN 的端點——而非 VEN 狀態）；Online 章改顯示計數與版本分布摘要，不再逐台列出（逐台明細見 XLSX/CSV 匯出；Offline／Lost 各桶的 HTML 仍保留逐台明細） |
| Policy Usage | `report policy-usage` | 每條 rule 的命中分析，找出未使用規則；Unused Rules 章的 HTML 僅顯示前 50 筆（完整清單——上限為分析資料層的 1000 筆——見 XLSX/CSV 匯出）；三張 draft policy 風險表（可視性風險、draft 衝突、draft 覆蓋）已合併為一張「高風險流量配對」表並新增風險類型欄 |
