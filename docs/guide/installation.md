...
| **作業系統** | RHEL / Rocky Linux 8+、Ubuntu 22.04+、Debian 12+（glibc >= 2.28）。 |
| **磁碟** | 離線安裝包：安裝根目錄所在磁碟至少 2 GB 可用空間。 |
...
CPython 3.12 執行環境，並下載 Linux（manylinux）wheel。
...
離線安裝的 `install.sh` 會自動複製並設定這些檔案。若需自訂安裝路徑，
...
`install.sh` 偵測到目標路徑已存在 `config/config.json` 即視為升級，並套用以下防呆：
...
1. **Downgrade 守衛**：拒絕安裝比已裝版本更舊的 bundle（比對 bundle `VERSION`
   ...
   蓋過已被較新版本遷移過的 DB。確有需要時以
   `sudo ./install.sh --allow-downgrade` 覆寫。
2. **服務停機行為**：若服務正在執行中，安裝腳本會自動停止服務再覆寫檔案，避免
   新舊程序與新舊 site-packages 混用造成 torn state；安裝完成後需由操作者自行
   重新啟動（`sudo systemctl restart illumio-ops`）。
3. **pristine runtime 與全量重裝**：以 `rsync --delete` 還原 bundle 內建 Python
   runtime 並全量重裝 bundle 內的 wheel，升級後只更新 `*.example` 範本。升級後
   機器上的相依版本必定與 bundle 一致，新版已移除的檔案（含被重新命名或刪除的
   `src` 模組）也會被清掉。
