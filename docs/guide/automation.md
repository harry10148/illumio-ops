...
- GUI：`src/gui/routes/rule_scheduler.py`（`/api/rule_scheduler/*`），對應 gui-tour.md
...
排程建立時，引擎會把一段**固定英文**的排程註記寫入 PCE 該 rule 的 `description` 欄位（例如
`[📅 Recurring: Mon,Tue,Wed 08:00-18:00 (UTC+8) Enable in window]` 或
`[⏰ Expires: 2026-08-01 00:00]`），刪除排程會盡力清除該註記。**註記固定用英文**，不論
GUI／CLI 當下顯示語言為何——因為這段文字會落在 PCE 的 description 裡，日後被 Policy Usage／
Audit 等報表原樣讀出，用操作者當下語言寫入會汙染其他語言的報表輸出。
...
- **one_time**：到期前 `target=True`（維持啟用）；一旦當下時間超過 `expire_at`，引擎立即把
  目標**停用並 provision**、清除 PCE 上的排程註記、把這筆排程從 `rule_schedules.json` 刪除
  （過期排程不會留在清單裡）。
...
- 5 個一律註冊，其餘 9 個依設定條件啟用（TLS 1 個、pce_cache 系 7 個、SIEM 1 個）；本篇
  只涉及 `tick_rule_schedules`，其餘 job 的資料流細節分別屬於 cache-maintenance.md、
  siem.md、monitoring-alerts.md、troubleshooting.md（部分尚待建立，見各篇說明）。
...
- job never-ran／overdue 判讀與故障排除：見 troubleshooting.md（尚待建立）
- pce_cache 系 job（ingest／aggregate／retention／archive）的容量與維運細節：見
  cache-maintenance.md（尚待建立）
- SIEM dispatch job 與 DLQ：見 siem.md（尚待建立）
