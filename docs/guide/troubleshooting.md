...
verified_against:
  ...
  - src/report/snapshot_store.py
  - src/report/posture.py
  - src/cli/report.py
...
`policy-usage`／`draft-policy` 七個子命令的 `_REPORT_FORMATS` 都列了它，`draft-policy` 走的是與 `traffic`
相同的 `generate_traffic_report()`／`ReportGenerator.export()` 路徑，見 `src/cli/report.py`），但對應
exporter（`ReportGenerator.export()`／`VenStatusReport.export()` 等）目前**沒有實作 `pdf` 分支**——選這個
格式會靜默不輸出任何檔案（exit code 仍為 0）。這是現況程式落差，不是操作錯誤。
...
### 4.5 症狀：儀表板／Security Posture 資料看似過期

**判讀**：Dashboard 首頁的 KPI 快照（`/api/dashboard/snapshot` 讀的 `latest_snapshot.json`）與 Security Posture
分數（`/api/dashboard/overview` 讀 `snapshot_store` 的 `traffic` 型快照，`posture.generated_at` 即其時戳）都是
在**產生 traffic-family 報表**時、由 `ReportGenerator.export()`（`src/report/report_generator.py`）一併寫入
的副產品——只有 `report traffic`／`report security`／`report inventory`／`report draft-policy` 這四個子命令（皆
走 `generate_traffic_report()`）會觸發這個寫入路徑；`audit`／`ven-status`／`policy-usage`／`rule-hit-count`／
`readiness` 各自用不同的 Generator class 匯出，不會更新這兩份快照。GUI 首頁的 **Refresh** 鈕與 auto-refresh 只
是重新抓取這些檔案目前已有的內容，**不會**觸發任何報表運算，時戳自然不會前進（詳見
[gui-tour.md](gui-tour.md)「1) Dashboard」一節）。Security Posture 磚塊在 `posture.generated_at` 超過 30 分鐘、
VEN Health 大數字在 `ven.computed_at` 超過 15 分鐘時會變灰並標示「stale」，這是設計行為，不代表資料損毀。

**處置**：

1. 要讓 Dashboard／Security Posture 的資料前進，必須實際跑一次上述四個 traffic-family 報表其中之一，例如：

   ```bash
   illumio-ops report traffic --format html
   ```

   `security`／`inventory`／`draft-policy` 效果相同（皆會覆寫 `latest_snapshot.json` 與 posture 用的快照），選
   哪個子命令依你原本想產生的報表內容而定；透過 GUI 觸發對應報表產生也會有相同效果。
2. 若要定期自動更新而不想每次手動觸發，設定 `report_schedules` 讓排程器（`tick_report_schedules` job，每 60
   秒 tick 一次）定期產生 traffic-family 報表，鍵值見 [configuration.md](configuration.md)「report ／
   report_schedules」一節、job 對照表見 [automation.md](automation.md) §3。
3. 舊文件提到的 `illumio-ops report run --format snapshot` **不存在**——沒有 `report run` 這個子命令，
   `--format` 也沒有 `snapshot` 這個值（合法值僅 `html`／`csv`／`pdf`／`xlsx`／`all`，見 §4.3）；不要嘗試這個
   命令，改用上面第 1 點的實際子命令。
