...
### 4.3 症狀：`--format pdf` 被 CLI 拒絕（或在舊版沒有產生任何檔案）

**判讀**：`pdf` 從未有任何 exporter 實作。2026-07-17 起它已從 `--format` 選項清單移除
（`_REPORT_FORMATS`＝`html`／`csv`／`xlsx`／`all`，見 `src/cli/report.py`）——現版傳入 `pdf`
會被 CLI 以無效選項直接拒絕，這是預期行為。若你在**更舊的版本**上選了 `pdf`，行為是靜默
不輸出任何檔案且 exit code 仍為 0——那是舊版的程式落差，升級即解。
...
3. 舊文件提到的 `illumio-ops report run --format snapshot` **不存在**——沒有 `report run` 這個子命令，
   `--format` 也沒有 `snapshot` 這個值（合法值僅 `html`／`csv`／`xlsx`／`all`，見 §4.3）；不要嘗試這個
   命令，改用上面第 1 點的實際子命令。
