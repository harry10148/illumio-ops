...
| After upgrade: old config still loaded | `install.sh` / `install.ps1` deliberately preserves the entire `config/` tree | Diff against the new template: `diff /opt/illumio-ops/config/config.json.example /opt/illumio-ops/config/config.json` and merge new keys manually. |
