...
| **Event Pipeline** | `src/events/` | Called during the event stage of the real-time monitor engine | Raw PCE events | Normalization, dedup, throttling, classification (known/unknown), shadow comparison, statistics, runbook mapping |
...
2. **policy_decision (`pd`)**: the flow's `pd` is numericalized as `0=allowed`, `1=potentially_blocked`, `2=blocked` (inferred from the `policy_decision` string when there is no `pd` field). A rule's `pd` defaults to `-1` (event-type) or `3` (traffic-type); **both `-1` and `3` mean "any PD"**, while any other value must match exactly to hit.
...
Event processing has eight stages: **poll → normalize → dedup → throttle → classify (known/unknown) → shadow-compare → stats → runbooks**. The package docstring describes itself as "inspired by illumio-pretty-cool-events".
...
- **`mod_drift`**: baseline drift — the app→app connection pairs added/disappeared compared with the previous run (a pure signature comparison).
