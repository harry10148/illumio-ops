...
輸出 `dist/illumio-ops-<version>-offline-linux-x86_64.tar.gz`；`<version>` 由 `resolve_version.sh` 解出。腳本內嵌 python-build-standalone 的下載 URL 與 SHA256 pin（`PBS_TAG`／`PBS_PYTHON`／`PBS_SHA256_LINUX_X86_64`），升級內嵌 Python 版本時三個欄位要一起改、一起送同一個 commit，且 SHA256 要從發布來源以外的管道（GPG／Sigstore）重新核對，避免 same-origin 供應鏈攻擊。
