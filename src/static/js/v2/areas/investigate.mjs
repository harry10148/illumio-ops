  ...
  return [
    ...
    col("act", t("gui_actions"), widthCell(170, function (r) {
      ...
      if (!r.srcHref && !r.dstHref) {
        ...
        cell.appendChild(btn("btn danger", t("gui_btn_isolate"), function () { onIsolate(r); }));
      }
      return cell;
    })),
  ];
...
async function mountTraffic(root, ctx) {
  ...
  if (alertId) {
    try {
      const det = await api.load("alert_detail", { id: alertId });
      if (ctx.stale() || state.torn) return;
      hub.alert = det && det.alert ? det.alert : null;
    } catch (e) {
      hub.reason = errText(e && e.data ? e.data : e);
    }
    try {
      const q = await api.load("alert_traffic_query", { id: alertId });
      if (ctx.stale() || state.torn) return;
      hub.query = q && q.query ? q.query : null;
    } catch (e) {
      const data = e && e.data ? e.data : {};
      hub.reason = data.code === "not_traffic" ? t("gui_hub_not_traffic")
        : data.code === "rule_missing" ? t("gui_hub_rule_missing")
        : errText(data && data.error ? data : e);
    }
  } else if (inlineF) {
    try {
      const raw = inlineF.indexOf("ov3.f.") === 0 ? window.sessionStorage.getItem(inlineF) : atob(inlineF.replace(/-/g, "+").replace(/_/g, "/"));
      hub.query = JSON.parse(raw || "{}");
    } catch (e) {
  ...
  await withErrorCard(main, "traffic (" + TRAFFIC_SNAPS.length + ")",
    ...
    function (d) {
      ...
      state.filters = {};
      if (hub.query) {
        // scalar keys first (rules that predate the FilterBar keep port /
        // dst_label at top level), then the FilterBar dict on top
        const q = hub.query;
        const scalars = {};
        Object.keys(q).forEach(function (k) {
          if (["type", "pd", "threshold_window", "filters"].indexOf(k) < 0 && q[k] !== null && q[k] !== undefined && q[k] !== "") scalars[k] = q[k];
        });
        state.filters = Object.assign({}, scalars, q.filters || {});
        if (q.pd !== undefined && q.pd !== null && String(q.pd) !== "-1") state.pd = String(q.pd);
        if (q.threshold_window) state.mins = String(q.threshold_window);
        state.sort = q.type === "bandwidth" || q.type === "volume" ? q.type : "connections";
      }
