...
//
// PORT OF design/v2/mockup/js/components/progress.mjs, verbatim except:
//   - i18n keys renamed v2_progress_* -> gui_progress_*.
//   - handle.destroy is added as an alias for handle.close (teardown
//     contract): both remove the card immediately, whereas done() removes it
//     on a delay so the "done" state is visible for a moment first.
...
export const progress = {
  /** start(label, steps) -> {el, step(n), done(), fail(msg), collapse(on), close(), destroy()} */
  start(label, steps) {
    ...
    const toggle = el("button", { class: "iconbtn", type: "button", title: t("gui_progress_collapse"), text: "–" });
    toggle.addEventListener("click", function () {
      const now = card.dataset.collapsed === "true";
      card.dataset.collapsed = now ? "false" : "true";
      toggle.textContent = now ? "–" : "+";
      toggle.title = now ? t("gui_progress_collapse") : t("gui_progress_expand");
    });
