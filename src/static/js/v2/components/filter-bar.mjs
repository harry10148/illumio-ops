...
// was already true in the mockup, unchanged here. When an area mounts a
// FilterBar inside a drawer body, the opener wires the two together itself:
//   const handle = drawer.open({ title, body: hostEl });
//   const fb = createFilterBar(hostEl, opts);
//   handle.onClose(() => fb.destroy());
// filter-bar.mjs does not know about drawer.mjs (it imports nothing, per
// constraint 1 above) — the opener is what connects the two.
