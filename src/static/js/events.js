...
function _renderEventViewerRows(items, append = false) {
  ...
  const rowsHtml = items.map((item) => {
    ...
    return `
      <tr data-event-id="${escapeHtml(item.event_id)}" data-severity="${escapeHtml(sev)}" data-action="selectEventViewerRow" data-args='${escapeHtml(JSON.stringify([item.event_id]))}' style="cursor:pointer;"><td><span class="ev-sev-bar ${sevClass}" aria-hidden="true"></span><span class="ev-time">${escapeHtml(formatDateZ(item.timestamp || ''))}</span></td><td><div class="ev-evtype-name">${escapeHtml(item.event_type || '')}</div>${(normalized.resource_type || normalized.resource_name) ? `<div class="ev-evtype-meta">${escapeHtml(normalized.resource_type || '')}${normalized.resource_name ? ' | ' + escapeHtml(normalized.resource_name) : ''}</div>` : ''}</td><td>${_evBadge(item.status || 'n/a', _evStatusTone(item.status))}</td><td>${_evCompactText(normalized.actor)}</td><td>${_evCompactText(normalized.target_name)}</td><td>${_evCompactText(normalized.action)}</td></tr>
    `;
  }).join('');
