...
// the chrome on every route). app.mjs owns the attach/detach; this module just
// renders the rail and keeps its audit opener quiet while detached.
//
// The tone of every light is transcribed from the shipping GUI so the mockup
// cannot invent a nicer reality than the product shows:
//
...
//                 (_buildOvCards) — dlq > 0 renders card-warn. DESIGN-ADDED (no
//                 source backing): siem_success_1h < 95 also escalates to warn.
//                 The product only puts that threshold in a reason string
//                 (_pipelineReasons, 1399-1401) and colours the card by verdict
//                 alone — the rail escalates on it deliberately, because a light
//                 that stays green while one dispatch in twenty fails is not
//                 doing its job. Labelled here and in the task report so nobody
//                 mistakes it for transcription.
...
function jobsLight(ov) {
  ...
  bad.forEach(function (j) {
    ...
    light.reasons.push(tf("v2_health_job_line", {
      job: j.job_id,
      status: status,
  ...
  if (!bad.length) light.reasons.push(tf("v2_health_jobs_ok", { ok: okCount, total: jobs.length }));
...
function pceLight(st, ov) {
  ...
  if (failures > 0) {
    const line = tf("v2_health_pce_failures", { n: failures, stage: p.last_error_stage || "—" });
    light.reasons.push(line);
    summary = line;
  }
  if (p.last_success) {
    light.reasons.push(tf("v2_health_pce_last_success", { age: since(p.last_success, ov && ov.as_of) }));
  }
  if (p.last_error) {
    light.reasons.push(tf("v2_health_pce_last_error", {
      stage: p.last_error_stage || "—",
      message: firstLine(p.last_error, 120),
    }));
    light.reasons.push(t("v2_health_pce_sticky"));
  }
  if (!light.reasons.length) light.reasons.push(t("v2_health_clear"));
...
function lagLight(ov) {
  ...
  lags.forEach(function (l) {
    if (l.level === "error" || l.level === "warning") {
      ...
      light.reasons.push(tf("v2_health_lag_line", { source: l.source, age: dur(l.lag_s) }));
    }
  });
  if (!lags.length) light.reasons.push(t("v2_health_clear"));
...
function siemLight(ov) {
  ...
  // DESIGN-ADDED escalation (see the header note) — the source only reports this
  // as text; the rail lets it move the LED.
  ...
  light.reasons.push(tf("v2_health_siem_queue", {
    state: p.siem_idle ? t("v2_health_queue_idle") : t("v2_health_queue_active"),
  }));

  return light.set(tn,
    (pct === null || pct === undefined ? "—" : pct + "%"),
    t("gui_ov_dlq") + " " + (Number(p.dlq) || 0) + " · " + (p.siem_idle ? t("v2_health_queue_idle") : t("v2_health_queue_active")));
}
...
function channelsLight(st) {
  ...
  failed.forEach(function (c) {
    light.reasons.push(tf("v2_health_chan_fail", { name: c.display_name || c.name, status: c.last_status }));
  });
  light.reasons.push(tf("v2_health_chan_live", { live: live.length, total: chans.length }));

  return light.set(failed.length ? "crit" : (live.length ? "ok" : "neutral"),
    live.length + "/" + chans.length,
    live.map(function (c) { return c.display_name || c.name; }).join(" · ") || t("v2_health_clear"));
}
...
function buildPopover(light, close) {
  const pop = el("div", { class: "popover rail-pop", "data-tone": light.tone, role: "dialog" },
    el("h4", { text: light.label + " · " + t("v2_health_reasons") }),
    ...
    el("div", { class: "pop-foot" },
      el("button", {
        class: "btn link",
        type: "button",
        text: t("v2_health_goto") + " " + light.route,
...
export const healthbar = {
  /** render(statusSnap, overviewSnap) -> HTMLElement (the whole rail, XC-01) */
  render(statusSnap, overviewSnap) {
    ...
    // while the rail is detached — app.mjs only attaches it on #/overview, and
    // opening popovers in a detached tree would inflate the audit's "opened"
    // count on every other route for surfaces nobody can see.
  ...
  /** mount(host, statusSnap, overviewSnap) — replaces host's contents. */
