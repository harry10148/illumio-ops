...
def test_get_ip_lists_returns_definitions():
    ...
    c._api_get.assert_called_once_with(
        "/orgs/1/sec_policy/active/ip_lists?max_results=10000")
...
def test_get_label_groups_returns_members():
    ...
    assert out == payload
    c._api_get.assert_called_once_with(
        "/orgs/1/sec_policy/active/label_groups?max_results=10000")
...
def test_get_services_returns_definitions():
    ...
    assert out == payload
    c._api_get.assert_called_once_with(
        "/orgs/1/sec_policy/active/services?max_results=10000")
