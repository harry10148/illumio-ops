"""Regression test: dashboard_v2.js must populate the 6 Phase 3.1 story-stat
...
JS = ROOT / "src" / "static" / "js" / "dashboard_v2.js"


def test_ensure_layout_does_not_reassign_card_ids():
    """ensureDashboardLayout must not change `cards[N].value.id` — index.html
    ...
    assert "value.id = 'd-cooldown'" not in js, (
        "ensureDashboardLayout must not reassign value.id to 'd-cooldown' — "
        "this overwrites Phase 3.1 story-card markup"
    )
    assert "value.id = 'd-pce-health'" not in js, (
        "ensureDashboardLayout must not reassign value.id to 'd-pce-health'"
    )
...
def test_load_dashboard_populates_six_story_stats():
    """v2 loadDashboard must call _dashboardSetCard for ALL 6 story-stat IDs."""
    js = JS.read_text(encoding="utf-8")
    required_ids = ['d-rules', 'd-health', 'd-event-poll',
                    'd-dispatch', 'd-unknown', 'd-suppressed']
    missing = [
        i for i in required_ids
        if f"_dashboardSetCard('{i}'" not in js
    ]
    assert not missing, (
        f"loadDashboard must populate these IDs via _dashboardSetCard: {missing}"
    )
