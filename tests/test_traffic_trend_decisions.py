...
def _seed_db(path):
    ...
    # The endpoint excludes the incomplete current day, so seed two COMPLETE past
    # days plus a "today" row that must NOT appear in the response.
    rows = [
        (day2, "allowed", 443, 50), (day2, "blocked", 3389, 5),
    ...
    with sessionmaker(engine)() as s:
        for day, action, port, fc in rows:
            s.add(PceTrafficFlowAgg(bucket_day=day, src_workload="a", dst_workload="b",
                                    ...
                                    bytes_total=fc * 1000))
        s.commit()
    return today.date().isoformat(), yest.date().isoformat(), day2.date().isoformat()


def _client(tmp_path):
    from src.gui import _create_app
    db = tmp_path / "cache.sqlite"
    today, yest, day2 = _seed_db(db)
    cm = MagicMock()
    ...
    return app.test_client(), today, yest, day2


def test_trend_splits_by_decision(tmp_path):
    client, today, yest, day2 = _client(tmp_path)
    ...
    # The incomplete current day is deliberately excluded (see endpoint docstring).
    assert today not in by_day
    assert by_day[day2]["allowed"] == 50
    assert by_day[day2]["potential"] == 0
    assert by_day[day2]["blocked"] == 5
    assert by_day[yest]["allowed"] == 100
    assert by_day[yest]["potential"] == 20
    assert by_day[yest]["blocked"] == 5
    # ordered oldest-first
    assert [b["ts"] for b in data["buckets"]] == [day2, yest]
